...
def paste_masks_v2():
    ...
    origin_ann_dir = join(root_dir, data_name)
    masked_im_dir = join(root_dir, f'{data_name}_with_masks_v10_seed{seed_used}')

    generated_bbox_dir = join(root_dir,f'{data_name}_generated_bbox{suffix}_seed{seed_used}')
    dst_dir = join(root_dir,f'{data_name}_generated_im{suffix}_ipadatper_copy_paste_by_cls_seed{seed_used}')
    ...
    for generated_bbox_path in tqdm(generated_bbox_paths):
        ...
        if sku_name not in particular_cats_ipadapter+particular_cats_copy_paste:
            continue
        ...
        if sku_name in particular_cats_ipadapter:
        ...
        elif sku_name in particular_cats_copy_paste:
            top_left = (x,y)
            masked_im, adjust_ann = copy_paste_bbox(generated_bbox, masked_im, original_bbox_path, padding, im_w, im_h, top_left)
            instance_bbox = adjust_ann
            copy_paste_cnt += 1
    ...
    print('copy_paste_cnt:', copy_paste_cnt)
    pass
